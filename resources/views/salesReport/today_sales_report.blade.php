    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="panel-heading " style="display: flex;justify-content: space-between;">
                    <h3 class="panel-title">Today Sales Report</h3>
                    ...
                    @php
                    $date = date("d-m-y");
                    $total = DB::table('order')->where('order_date',$date)->sum('total');
                    $sub_total = DB::table('order')->where('order_date',$date)->sum('sub_total');
                    $pay = DB::table('order')->where('order_date',$date)->sum('pay');
                    $due = DB::table('order')->where('order_date',$date)->sum('due');
                    $total_product = DB::table('order')->where('order_date',$date)->sum('total_products');
                    @endphp
